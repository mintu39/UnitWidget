ZOHO.embeddedApp.on("PageLoad", function () {
  ...
  const matchItem = vipPrimary.find(x => /\b\d{1,3}(,\d{3})?\s*(sq\s?ft|sqft)\b/i.test(x));
  if (!matchItem) return "";
  ...
  function detectUnitTypeFromTitleAndDescription(title = "", descriptionArray = []) {
    const text = (title + " " + (descriptionArray || []).join(" ")).toLowerCase();

    const typeMap = [
      { type: "Condominium", keywords: ["condo", "luxury condo", "condominium", "condo unit", "condo apartment"] },
      { type: "Townhouse", keywords: ["townhouse", "townhome", "end unit townhouse", "3-storey townhouse"] },
      { type: "Unit - Apartment Building", keywords: ["apartment", "unit", "suite", "corner unit", "high-rise", "low-rise"] },
      { type: "House", keywords: ["house", "detached", "bungalow", "single family", "entire house"] },
      { type: "Multi Unit - Above Ground", keywords: ["triplex", "fourplex", "main floor", "upstairs unit", "upper level in house", "corner suite"] },
      { type: "Stacked Townhouse", keywords: ["stacked townhouse", "stacked townhome", "upper stacked", "lower stacked"] },
      { type: "Student Room", keywords: ["student room", "student housing", "room near college", "room near university", "all-inclusive student room"] },
      { type: "Basement", keywords: ["basement", "basement apartment", "lower level", "basement suite", "walkout basement"] }
    ];
  ...
  function detectBasementIncluded(title = "", descriptionArray = []) {
    ...
    if (text.includes("basement unit") || text.includes("this is the basement")) {
      return "This is the basement";
    }
    if (text.includes("separate basement") || text.includes("basement access") || text.includes("private basement")) {
      return "Separate Unit";
    }
    if (text.includes("finished basement")) {
      return "Finished";
    }
    if (text.includes("partially finished basement") || text.includes("half finished basement")) {
  ...
  function detectEntranceType(descriptionArray = []) {
    ...
    if (
      text.includes("walk out") || text.includes("walkout") ||
      text.includes("walk-out") || text.includes("back entrance")
    ) {
      return "Private - Walk out (Back Entrance)";
    }
    if (text.includes("entrance through garage") || text.includes("garage access")) {
      return "Entrance Only via Garage";
    }
    if (
      text.includes("via main floor") || text.includes("through upstairs") ||
      text.includes("main unit access")
    ) {
      return "Entrance Only via Main floor Unit";
    }
    if (
      text.includes("no basement") || text.includes("above ground") ||
  ...
  function detectMaxOccupants(descriptionArray) {
    ...
    const match = combined.match(/(?:maximum|max|ideal for|up to)?\s*(\d{1,2})\s*(people|occupants)/i);
  ...
  function detectPropertyCondition(descriptionArray) {
    ...
    if (combined.includes("brand new") || combined.includes("never lived in")) {
      return "Brand New";
    } else if (combined.includes("renovated") || combined.includes("upgraded")) {
      return "Newly Renovated";
    } else if (combined.includes("moderate") || combined.includes("fair")) {
      return "Moderate";
    } else if (combined.includes("needs work") || combined.includes("fixer upper")) {
  ...
  function detectNumberOfLevels(descriptionArray = [], title = "") {
    ...
    const matchMap = [
      { value: "1", keywords: ["bungalow", "flat", "single level", "main floor", "ground level", "ranch", "one floor", "1 storey"] },
      { value: "2", keywords: ["duplex", "2-storey", "two-storey", "bi-level", "two levels", "upper/lower level", "split level"] },
      { value: "3", keywords: ["triplex", "3-storey", "three levels", "three floors"] },
      { value: "Shared", keywords: ["shared house", "student room", "co-living", "shared unit", "room for rent"] }
    ];
  ...
  function extractBasementDetails(descriptionArray = []) {
    const basementKeywords = ["basement", "lower level", "shared laundry", "separate entrance", "in basement"];
  ...
  function detectCheckboxFeatures(descriptionArray = [], attributes = []) {
    ...
    return {
      ...
      Natural_Sunlight: text.includes("natural light") || text.includes("sunlight") || text.includes("bright"),
      Fireplace_Common_Area: text.includes("fireplace") || text.includes("living room fireplace"),
      Fireplace_Bedroom: text.includes("fireplace in bedroom"),
      Upgraded_Bathrooms: text.includes("upgraded bathroom") || text.includes("modern bath"),
      Backsplash_Kitchen: text.includes("backsplash") || text.includes("tile wall behind stove") || text.includes("tile wall behind sink"),
      Upgraded_Kitchen: text.includes("renovated kitchen") || text.includes("modern kitchen"),
      Dishwasher_Included: appliances.includes("dishwasher") || text.includes("dishwasher included") || text.includes("built-in dishwasher")
    };
  }
  // Detect flooring type in common area based on description
  function detectFlooringCommonArea(descriptionArray = []) {
    const text = (descriptionArray || []).join(" ").toLowerCase();
    const flooringOptions = ["vinyl", "laminate", "carpet", "hardwood", "tile", "concrete"];
  ...
  function detectCeilingHeight(descriptionArray = []) {
    ...
    const match = text.match(/(7|8|9|10|15)\\s?(foot|ft)[\\s-]?(ceiling|ceilings)?/);
  ...
  function detectWindowCoverings(descriptionArray = []) {
    ...
    const matchMap = [
      { keywords: ["roller shade", "roller blind", "roller blinds", "roller shades"], value: "Roller" },
      { keywords: ["1 inch blinds", "1'' blind", "1\" blind", "1 inch aluminum"], value: "1'' Aluminum Blinds" },
      { keywords: ["2 inch blind", "2'' blind", "2 inch faux", "2 inch vinyl"], value: "2'' Faux Blinds" },
      { keywords: ["california shutter", "shutters"], value: "California Shutters" },
      { keywords: ["vertical blind", "verticals"], value: "Vertical" },
      { keywords: ["no window covering", "without window covering", "no blinds"], value: "No Window Coverings" },
  ...
  function extractBedroomsSmart(vipPrimary = [], title = "", description = []) {
    const combinedText = (title + " " + (description || []).join(" ")).toLowerCase();
  ...
  function detectDenAsBedroom(descriptionArray = []) {
    ...
    if (text.includes("den can be used as bedroom") || text.includes("den can be used as a bedroom")) {
      return "Yes";
    }
  ...
  function detectClosetType(descriptionArray = []) {
    ...
    if ((text.includes("his and hers") || text.includes("his & hers")) && text.includes("all bedrooms")) {
    ...
    if ((text.includes("his and hers") || text.includes("his & hers")) && (text.includes("primary") || text.includes("master"))) {
      return "His and Hers (Primary Bedroom Only)";
    }

    if (text.includes("walk-in") && (text.includes("primary") || text.includes("master"))) {
      return "Walk-in (Primary Bedroom Only)";
    }

    if (text.includes("no closets") || text.includes("no closet") || text.includes("no storage")) {
      return "No Closets";
    }

    if (text.includes("some rooms have no closet") || text.includes("not all bedrooms have closets")) {
  ...
  function detectEnSuiteBathrooms(descriptionArray = []) {
    ...
    if (
      (text.includes("ensuite") || text.includes("en-suite")) &&
      (text.includes("both rooms") || text.includes("all rooms") || text.includes("multiple rooms") || text.includes("each bedroom"))
    ) {
  ...
  function extractBathroomsSmart(vipPrimary = [], title = "", description = []) {
    const combinedText = (title + " " + (description || []).join(" ")).toLowerCase();
  ...
  function detectBathroomCountertop(descriptionArray = []) {
    ...
    if (text.includes("granite") || text.includes("quartz") || text.includes("stone") || text.includes("marble")) {
      return "Stone";
    }

    if (text.includes("laminate") || text.includes("formica") || text.includes("melamine")) {
      return "Formica";
    }
  ...
  function detectShowerType(descriptionArray = []) {
    ...
    const hasStanding = text.includes("walk-in shower") || text.includes("standing shower");
    const hasSeat = text.includes("shower seat") || text.includes("shower with seat");
    const hasBathtubShower = text.includes("bathtub with shower") || text.includes("tub shower combo");
    const hasFreeStandingTub = text.includes("free standing tub") || text.includes("freestanding bathtub");

    const isMultiple = text.includes("multiple bathrooms") || text.includes("multiple ensuites") || text.includes("in multiple rooms");

    // Priority order:
    if (hasStanding && hasSeat && isMultiple) return "Standing Shower (With Seat) in Multiple Rooms";
    ...
    if (hasFreeStandingTub && hasStanding && isMultiple) return "Free Standing Bathtub & Standing Shower in Multiple Rooms";
    if (hasFreeStandingTub && hasStanding) return "Free Standing Bathtub & Standing Shower";
    if (hasFreeStandingTub) return "Free Standing Bathtub";
    if (hasBathtubShower && isMultiple) return "Bathtub in Multiple Rooms with Shower";
    if (hasStanding && hasBathtubShower && isMultiple) return "Standing Shower & Bathtubs with Shower in Multiple Rooms";
  ...
  function detectApplianceFinish(descriptionArray = []) {
    ...
    if (text.includes("paneled appliances") || text.includes("built-in panel") || text.includes("panel ready")) {
      return "Paneled";
    }
  ...
  function detectKitchenCountertops(descriptionArray = []) {
    ...
    if (stoneKeywords.some(keyword => text.includes(keyword + " kitchen") || text.includes(keyword + " countertop"))) {
      return "Stone";
    }

    // Formica keywords
    const formicaKeywords = ["laminate", "formica"];
    if (formicaKeywords.some(keyword => text.includes(keyword + " kitchen") || text.includes(keyword + " counter"))) {
      return "Formica";
    }
  ...
  function detectBedroomFlooring(descriptionArray = []) {
    ...
    const flooringTypes = ["vinyl", "laminate", "carpet", "hardwood", "tile", "concrete"];
  ...
  function detectBalconyLocation(descriptionArray = []) {
    ...
    const mapping = [
      { value: "Bedroom and Living Area", keywords: ["balcony in bedroom and living room", "balcony off bedroom and living room"] },
      { value: "Both Bedrooms and Living Area", keywords: ["both bedrooms and living room", "balcony from both bedrooms and living"] },
      { value: "Both Bedrooms", keywords: ["balcony from both bedrooms", "balcony off both bedrooms"] },
      { value: "Front Hallway & Living area", keywords: ["balcony in front hallway and living room", "balcony off front hallway and living"] },
      { value: "Front Hallway & Kitchen", keywords: ["balcony in front hallway and kitchen", "balcony off front hallway and kitchen"] },
      { value: "Front Hallway", keywords: ["balcony in front hallway", "balcony off hallway", "hallway balcony"] },
      { value: "Living Area", keywords: ["balcony from living room", "living room access to balcony", "balcony off living room"] },
      { value: "Bedroom", keywords: ["balcony off bedroom", "balcony in bedroom"] },
      { value: "Bathroom", keywords: ["balcony in bathroom", "bathroom access to balcony"] }
    ];
  ...
  function detectBackyard(descriptionArray = []) {
    ...
    if (text.includes("shared backyard") || text.includes("shared green space")) {
      return "Shared";
    }
    if (text.includes("no backyard access") || text.includes("no access to backyard") || text.includes("no yard access")) {
      return "No Backyard";
    }
  ...
  function detectBackyardFenced(descriptionArray = [], backyardValue = "") {
    ...
    if (text.includes("unfenced") || text.includes("open yard") || text.includes("no fence")) {
      return "No";
    }
  ...
  function detectView(descriptionArray = []) {
    ...
    const hasBackyard = text.includes("backyard view") || text.includes("courtyard") || text.includes("overlooks backyard");

    if (hasLake && hasCity && hasConservation) return "Lake, City, and Conservation";
  ...
  function detectParkingDetails(descriptionArray = [], title = "") {
    ...
    if (text.includes("street parking only") || text.includes("only street parking")) {
    ...
    if (text.includes("garage and outdoor") || text.includes("parking garage and outdoor lot")) {
  ...
  function extractStorageUnits(descriptionArray = []) {
    const text = (descriptionArray || []).join(" ").toLowerCase();
    const match = text.match(/(?:includes|with)?\s*(\d+)\s*(locker|storage|storage unit|storage locker)/);
  ...
  function extractLockerLevelAndNumber(descriptionArray = []) {
    ...
    const pattern = /locker\s*(on)?\s*(level\s*)?(p\d|b\d|[a-z]?\d)?\s*[#-]?\s*(\d{1,4})/i;
  ...
  function extractUtilityResponsibility(descriptionArray = []) {
    ...
    if (text.includes("tenant pays all") || text.includes("tenant responsible for all utilities")) {
      return "100%";
    }

    if (text.includes("utilities included") || text.includes("all utilities included")) {
      return "0%";
    }
  ...
  function detectHomeInsurance(descriptionArray = []) {
    const insuranceMap = {
      ...
      eglantan_scot_real_estate_mangment_inc: "EGLANTAN SCOT REAL ESTATE MANGMENT INC",
      entegrus: "Entegrus",
      ...
      north_blenheim_mutual_insurance_company: "North Blenheim Mutual Insurance Company",
      ...
      ayr_farmers_mutual_insurance_company: "Ayr Farmers Mutual Insurance Company",
      ...
      canadian_premier_life_insurance_company: "Canadian Premier Life Insurance Company",
      ...
      mcclelland_insurance_brokers_limited: "McClelland Insurance Brokers Limited",
      mcconville_omni_insurance_brokers_ltd: "McConville Omni Insurance Brokers Ltd",
      merit_insurance: "Merit Insurance",
      middlesex_mutual: "Middlesex Mutual",
      multi_risk_insurance_financial_group_inc: "Multi Risk Insurance & Financial Group Inc.",
      ...
      premier_canada_assurance_managers_ltd: "Premier Canada Assurance Managers. Ltd.",
      ...
      security_national_insurance_company: "Security National Insurance Company",
  ...
  function extractInsurancePolicyNumber(descriptionArray = []) {
    const text = (descriptionArray || []).join(" ").toLowerCase();
    const pattern = /(policy\s*#|insurance\s*(reference|policy))[^0-9]{0,10}(\d{7,10})/i;
    const match = text.match(pattern);
...
function extractStreetName(location = "", firstName = "", title = "") {
  ...
  const streetTypes = [
    ...
    "drive", "dr", "lane", "ln", "court", "ct", "crescent", "cr",
    "terrace", "way", "trail", "place", "pkwy"
  ].join("|");

  // Match optional number, followed by street name + suffix
  const regex = new RegExp(`(?:\\d{1,5}\\s+)?([a-z\\s]+?\\s(?:${streetTypes}))\\b`, "i");
...
function capitalizeWords(str) {
  ...
  function detectWaterProvider(descriptionArray = []) {
    const providers = [
      "Arthurs Fuel", "City of Oakville", "City of Oshawa", "City of Pickering", "City of Tecumseh",
      "Edoes", "Ersolutions", "Peel Water", "Utilities Kingston", "Wyse", "4K", "Alectra",
      "Aquaman Bulk Water Supply", "Bradford West Gwillimbury", "Brantford Utilities", "Burlington Hydro",
      "Carma", "City of Barrie", "City of Brantford", "City of Burlington", "City of Cambridge", "City of Guelph",
      "City of Kitchener", "City of London", "City of Niagara Falls", "City of Orillia", "City of Ottawa",
      "City of Port Colborne", "City of Richmond hill", "City of St. Catharines", "City of Thorold", "City of Toronto",
      "City of Vaughan", "City of Waterloo", "City of Whitby", "Clean Cut Energy", "Cricket Energy",
      "Crown Crest Capital", "Durham Region", "Earth Power", "Elexicon Energy", "Enbridge", "Enersource",
      "Enova", "Entegrus Powerlines", "Enwin", "ERTH Power", "Essa Township", "Essex power lines", "Fernsby",
      "Fort Erie - Water and Waste Services", "Granbridge Energy", "Guelph Hydro", "Haldimand County",
      "Haldimand County Water and Wastewater", "Hydro One", "Included", "InnPower", "King City",
      "Kitchener Utilities", "Lakershore Power Line", "Linc", "London Hydro", "Mersey", "Metergy Solutions",
      "Middlesex center county", "Municipality of Strathroy", "ND Energy", "Newmarket Hydro",
      "Newmarket Tay Power Distribution", "Newmarket-Tay Power Distribution", "Oakville Hydro", "Ontario Power",
      "Peel Region", "Power Stream", "Priority", "Provident", "Reliance home comfort", "Spectra", "Springwater Region",
      "The City of Georgina", "To Be Determined (TBD)", "Town of Aurora Water Department",
      "Town of Bradford West Gwillimbury", "Town of East Gwillimbury", "Town of Fort Erie", "Town of Georgina",
      "Town of Grimsby", "Town of New Tecumseth", "Town of Pelham", "Town of Stouffville", "Town of Tillsonburg",
      "Township of Springwater", "Township of Woolwich", "Veridian", "Wasaga Distribution Inc", "Wastewater",
      "Waterloo North Hydro", "Welland Municipality", "Welland Water", "Welland Water/Wastewater", "Wyse Meter",
      "York Region"
    ];
  ...
  function detectGasProvider(descriptionArray = []) {
    const providers = [
      "Arthurs Fuel", "City Of Waterloo", "Oakville Hydro", "Utilities Kingston", "4K", "Alectra",
      "Bradford West Gwillimbury", "Carma", "City of Kitchener", "Clean Cut Energy", "Cricket Energy",
      "Crown Crest Capital", "Direct Energy", "Earth Power", "Elexicon Energy", "Enbridge", "Enercare",
      "Enersource", "Enova", "ERTH Power", "Hydro One", "Included", "Kitchener Utilities", "London Hydro",
      "Mersey", "Metergy Solutions", "N/A", "Newmarket Tay Power Distribution", "Newmarket-Tay Power Distribution",
      "Ontario Power", "Power Stream", "Priority", "Propan", "Provident", "Reliance home comfort", "TBD",
      "Toronto Hydro", "Toronto Water", "Town of Tillsonburg", "UnionGas", "Waterloo North Hydro"
    ];
    ...
    if (text.includes("enbridge gas") || text.includes("union gas")) return "Enbridge";
...
// Detect hot water tank provider based on description
  function detectHotWaterTankProvider(descriptionArray = []) {
    const providers = [
      "Arthurs Fuel", "Evolve", "GSW", "Rinnai", "Utilities Kingston", "4K", "4K Energy", "Aire One",
      "Alectra", "Allianz", "Bradford West Gwillimbury", "Carma", "City of Kitchener", "City of Pickering",
      "Clean Cut Energy", "Cricket Comfort", "Cricket Energy", "Cricket Home Comfort", "Crown Crest Capital",
      "Direct Energy", "Earth Power", "Eco Star Home Services Co.", "Elevate", "Elexicon Energy", "En saving Inc",
      "Enbridge", "Enercare", "Enersource", "Enova", "Enpure", "Enresource", "ERTH Power", "Genesis Home Services",
      "Gian Inc", "Giant", "Green Planet", "HCSI Home Comfort", "Homecorp", "Htp", "Hydro One", "Included",
      "Kirin Air Systems Inc.", "Kitchener Utilities", "LaSalle Town", "LIMCAN", "London Hydro", "Mersey",
      "Metergy Solutions", "Midland", "Morenergy", "N/A", "National Home Services", "Newmarket Tay Power Distribution",
      "Newmarket-Tay Power Distribution", "Oakville Hydro", "Ontario Power", "Power Stream", "Priority", "Provident",
      "Radiant", "Region of Peel", "Reliance home comfort", "Rheem Guardian System", "Sandpiper", "Simply Green",
      "Superflue", "Tankless Water Heater", "TBD", "To be Determined (TBD)", "Toronto Hydro", "Toronto Water",
      "Town of Stouffville", "Town of Tillsonburg", "UnionGas", "Vista", "Waterloo North Hydro"
    ];
    ...
    if (text.includes("national home services")) return "National Home Services";
  ...
  return {
    AC_Inclusion: /air conditioning included|central ac|ac provided/.test(fullText),
    Heat_Inclusion: /heating included|heat paid by landlord|heat included/.test(fullText),
    Internet_Inclusion: /wifi included|internet included|free internet/.test(fullText),
  ...
  function extractUtilityNotes(descriptionArray = []) {
    ...
    const utilityLines = (descriptionArray || []).filter(line => {
      return /utility|utilities|hydro|gas|heat|internet|wifi|cable|phone|water/i.test(line);
    });
...
console.log("loggedInUserId",loggedInUserId);
    ...
    const numericPrice = data.price ? Number(data.price.replace(/[$,]/g, "")) : "";
      ...
      const basement = detectBasementIncluded(data.title || "", data.description || []);
      ...
      const features = detectCheckboxFeatures(data.description || [], data.vipAttributes?.attributes || []);
      ...
      const backyardFenced = detectBackyardFenced(data.description || [], backyard);
      const isPrivateTerrace = detectPrivateTerraceOrBackyard(data.description || []);
      const viewValue = detectView(data.description || []);
      const parkingSpaces = extractParkingSpaces(data.vipAttributes?.primary || [], data.title || "", data.description || []);
      const parkingDetails = detectParkingDetails(data.description || [], data.title || "");
      const parkingLevelNumber = extractParkingLevelNumber(data.description || []);
      ...
      const utilityInclusions = detectUtilityInclusions(data.description || [], data.vipAttributes?.attributes || []);
      const utilityNotes = extractUtilityNotes(data.description || []);
      document.getElementById("Utility_Notes").value = utilityNotes;
      document.getElementById("AC_Inclusion").checked = utilityInclusions.AC_Inclusion;
      document.getElementById("Heat_Inclusion").checked = utilityInclusions.Heat_Inclusion;
      document.getElementById("Internet_Inclusion").checked = utilityInclusions.Internet_Inclusion;
      document.getElementById("Cable_Inclusion").checked = utilityInclusions.Cable_Inclusion;
      document.getElementById("Phone_Inclusion").checked = utilityInclusions.Phone_Inclusion;
      document.getElementById("Hot_Water_Tank_Provider").value = hotWaterTankProvider;
      document.getElementById("Gas_Provider").value = gasProvider;
      document.getElementById("Water_Provider").value = waterProvider;
      document.getElementById("Electricity_Provider").value = electricityProvider;
      document.getElementById("Unit").value = extractUnit(data.title, data.firstName);
      document.getElementById("Street_Number").value = extractStreetNumber(data.location, data.firstName, data.title);
      document.getElementById("Street_Name").value = extractStreetName(data.location, data.firstName, data.title);
      document.getElementById("U_City").value = extractCity(data.location);
      document.getElementById("Province").value = extractProvince(data.location);
      document.getElementById("Postal_Code").value = extractPostalCode(data.location);
      document.getElementById("Mail_Box_Number").value = extractMailBoxNumber(data.description || []);
      document.getElementById("Country").value = "Canada";
      document.getElementById("Insurance_Policy_Number").value = insurancePolicyNumber;
      document.getElementById("Insurance_Home_Owner").value = insuranceCompany;
      document.getElementById("Utilities_Financial_Responsibility").value = utilityShare;
      document.getElementById("Locker_Level_and_Number").value = lockerLevelAndNumber;
      document.getElementById("Locker_Storage_Details").value = lockerDetails;
      document.getElementById("Number_of_Storage_Lockers").value = numberOfLockers || "";
      document.getElementById("Private_Garage").checked = detectPrivateGarage(data.description || []);
      document.getElementById("Walkout_To_Garage").checked = detectWalkoutToGarage(data.description || []);
      document.getElementById("Parking_Level_Number").value = parkingLevelNumber;
      ...
      document.getElementById("Private_Terrace_Backyard").checked = isPrivateTerrace;
      ...
      document.getElementById("Countertops_Kitchen").value = kitchenCountertops;
      document.getElementById("Appliance_Finishes").value = applianceFinish;
      document.getElementById("Shower_Type").value = showerType;
      document.getElementById("Countertops_Bathroom").value = bathroomCountertop;
      ...
      document.getElementById("Window_Coverings_Common").value = windowCoveringsCommon;
      document.getElementById("Window_Coverings_Bedroom").value = windowCoveringsBedroom;
      document.getElementById("Ceiling_Height").value = ceilingHeight;
      document.getElementById("flooringCommonArea").value = flooringCommonArea;
      document.getElementById("Corner_Unit").checked = features.Corner_Unit;
      document.getElementById("Central_Vacuum").checked = features.Central_Vacuum;
      document.getElementById("Penthouse").checked = features.Penthouse;
      document.getElementById("Natural_Sunlight").checked = features.Natural_Sunlight;
      document.getElementById("Fireplace_Common_Area").checked = features.Fireplace_Common_Area;
      document.getElementById("Fireplace_Bedroom").checked = features.Fireplace_Bedroom;
      document.getElementById("Upgraded_Bathrooms").checked = features.Upgraded_Bathrooms;
      document.getElementById("Backsplash_Kitchen").checked = features.Backsplash_Kitchen;
      document.getElementById("Upgraded_Kitchen").checked = features.Upgraded_Kitchen;
      document.getElementById("Dishwasher_Included").checked = features.Dishwasher_Included;
      document.getElementById("Basement_Details").value = basementDetails || "";
      ...
      document.getElementById("Interested_Unit_Title").value = data.title || "";
      document.getElementById("Unit_Type").value = unitType || "";
      document.getElementById("Approximate_Sq_Ft").value = sqFt || "";
      document.getElementById("Maximum_Occupants").value = maximumOccupants || "";
      ...
      document.getElementById("Property_Condition").value = propertyCondition || "";
      document.getElementById("Number_of_Levels").value = numberOfLevels || "";
  ...
  const leadData = {
    ...
    Lead_Priority_Level: document.getElementById("Lead_Priority_Level").value,
  ...
  const unitData = {
    ...
    Total_Area_Sq_Ft: parseInt(document.getElementById("Approximate_Sq_Ft").value) || null,
    Max_Occupants: parseInt(document.getElementById("Maximum_Occupants").value) || null,
    Property_Condition: document.getElementById("Property_Condition").value,
    Year_Built: parseInt(document.getElementById("Year_Last_Renovated").value) || null,
    ...
    Flooring_Common_Area: document.getElementById("flooringCommonArea").value,
    Ceiling_Hight: document.getElementById("Ceiling_Height").value,
    Window_Coverings: document.getElementById("Window_Coverings_Common").value,
    Window_Coverings_Common_Area: document.getElementById("Window_Coverings_Bedroom").value,
    Bedrooms: document.getElementById("Bedrooms").value,
    Bedroom_Layout: document.getElementById("Bedroom_Layout").value,
    Den_can_be_used_as_a_bedroom: document.getElementById("Den_As_Bedroom").value,
    ...
    Countertops_Bathroom: document.getElementById("Countertops_Bathroom").value,
    ...
    Flooring_in_Bedrooms: document.getElementById("Flooring_Bedrooms").value,
    Location_of_Balcony: document.getElementById("Balcony_Location").value,
    Backyard: document.getElementById("Backyard").value,
    Is_the_backyard_fenced: document.getElementById("Backyard_Fenced").value,
    Number_of_Parking_Spaces: parseInt(document.getElementById("Parking_Spaces").value) || 0,
    Parking_Details: document.getElementById("Parking_Details").value,
    parking_level_num: document.getElementById("Parking_Level_Number").value,
    View: document.getElementById("View").value,
    Number_of_Lockers: parseInt(document.getElementById("Number_of_Storage_Lockers").value) || null,
    Storage_Details: document.getElementById("Locker_Storage_Details").value || "",
    locker_level_and_number: document.getElementById("Locker_Level_and_Number").value || "",
    How_are_utilities_split: document.getElementById("Utilities_Financial_Responsibility").value || "",
    Insurance_Home_Owner: document.getElementById("Insurance_Home_Owner").value || "",
    Insurance_Home_Owners: document.getElementById("Insurance_Home_Owner").value || "",
    Insurance_Policy_Number: document.getElementById("Insurance_Policy_Number").value || "",
    ...
    Tons_of_Natural_Light: document.getElementById("Natural_Sunlight").checked,
    ...
    Upgraded_Bathrooms: document.getElementById("Upgraded_Bathrooms").checked,
    Upgraded_Back_Splash: document.getElementById("Backsplash_Kitchen").checked,
